...
package:
	...
	void retain()
	{
		...
		version(CL_VERSION_1_2)
			static if (TS == "cl_device_id")
				clRetainDevice(_object);

		// NOTE: cl_platform_id and cl_device_id aren't reference counted
		// TS is compared instead of T itself so it also works with T being an alias
		// platform and device will have an empty retain() so it can be safely used in this()
		static if (TS[$-3..$] != "_id")
		{
			mixin("cl_errcode res = clRetain" ~ toCamelCase(TS[2..$]) ~ (TS == "cl_mem" ? "Object" : "") ~ "(_object);");
	...
	void release()
	{
		...
		static if (TS[$-3..$] != "_id")
		{
			mixin("cl_errcode res = clRelease" ~ toCamelCase(TS[2..$]) ~ (TS == "cl_mem" ? "Object" : "") ~ "(_object);");
			mixin(exceptionHandling(
				["CL_OUT_OF_RESOURCES",		""],
				["CL_OUT_OF_HOST_MEMORY",	""]
			));
	...
	public @property cl_uint referenceCount() const
	{
		static if (TS[$-3..$] != "_id")
		{
			...
			mixin("return getInfo!cl_uint(CL_" ~ (TS == "cl_command_queue" ? "QUEUE" : (){char[] tmp = TS[3..$].dup; toUpperInPlace(tmp); return tmp;}()) ~ "_REFERENCE_COUNT);");
		}
		else
			return 0;
	}
...
unittest
{
	...
	// Object doesn't copied from collection!
	//CLDummy d = c[0];
